...
size_t Eleven::getSize() const { return size; }
...
Eleven::Eleven(Eleven &&other) noexcept
    : massiv(other.massiv), size(other.size) { // забираем тут у other
  other.massiv = nullptr;                      // обнуляемся
  other.size = 0;
}

Eleven::~Eleven() noexcept { delete[] massiv; }
...
Eleven Eleven::copy(const Eleven &other) const { return Eleven(*this); }
