...
private:
  ...
  static bool numberCheck(unsigned char num);
  void pruning();
...
public:
  ...
  Eleven(
      const size_t &n,
      unsigned char t = 0); // массив из n эл-в заполненный t //если n будет
                            // отриц преобразует в огромное положительное) int?
  ...
  // virtual ~Eleven() noexcept; // чистка, virtual чистка объектов производных
  // классов

  ~Eleven() noexcept;

  Eleven
  add(const Eleven &other) const; // возвращаем новый обхъект, передаем желемент
                                  // по ссылке, неменяется исходник
